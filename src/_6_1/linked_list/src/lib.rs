...
impl<T> LinkedList<T> {
    ...
    /*
    fn values(&self) -> impl std::iter::Iterator<Item = i8> {
        let item = std::iter::once(self.item);
        let next = std::iter::once(self.next);
        let prev = std::iter::once(self.prev);
        item.chain(next).chain(prev)
    }
    */
}
...
impl<T> std::iter::IntoIterator for LinkedList<T> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        LinkedListIntoIterator {
            list: self,
            index: 0,
        }
    }
}
struct LinkedListIntoIterator<T> {
    list: T,
    index: usize,
}
impl<T> Iterator for LinkedListIntoIterator<T> {
    ...
    fn next(&mut self) -> Option<T> {
        ...
        self.index += 1;
        if let Some(ref mut _n) = get_ptr_from_idx(&mut self.list.head, self.index, 0) { return Some(&mut (_n.item)) }
        else { return None; }
        /*
        fn get_ptr_from_idx<T>(ptr: &mut Option<Box<Node<T>>>, index: usize, count: usize) -> &mut Option<Box<Node<T>>> {
            if count < index {
