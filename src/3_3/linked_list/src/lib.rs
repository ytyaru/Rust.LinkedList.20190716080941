...
mod tests {
    ...
    // ノードが一つもないとき、remove_from_index(0)はエラーなし。remove_from_index(1)はエラーになる。out of indexという意味では0のときエラーにならなくて正解か？ 「ノードがひとつもないのに削除しようとしました」エラーはウザいので作りたくない。
     #[test]
    fn LinkedList_remove_from_index_out_of_index_blank_0() {
        let mut list: LinkedList<i32> = LinkedList::new();
        list.remove_from_index(0);
    }
    // ノードが一つもないとき、remove_from_index(0)はエラーなしで、remove_from_index(1)はエラーになる。    #[test]
    #[test]
    #[should_panic(expected = "Out of index. index値が大きすぎます。index: 1, 許容範囲: 0...0")]
    ...
    #[test]
    #[should_panic(expected = "Out of index. index値が大きすぎます。index: 2, 許容範囲: 0...0")]
