...
//impl std::iter::Iterator<T> for LinkedList<T> {
//impl Iterator<T> for LinkedList<T> {
//impl Iterator<T> for LinkedList<T> {
//impl Iterator for LinkedList<T> {
//impl Iterator for LinkedList {
impl<T> Iterator for LinkedList<T> {
    type Item = T;
//    fn next(&mut self) -> Option<Box<Node<Self::Item>>> {
...
//            std::mem::replace(&mut self.head, target.as_mut().unwrap().next);
//            std::mem::replace(&mut target.as_mut().unwrap().next, None);
