...
//            ptr = ptr.as_ref().unwrap().next;
    ...
    */
    /// 指定位置の参照を得る
//    fn nth_ref(&self, index: u32) -> &Option<Box<Node<T>>> {
//    fn nth_ref(&self, index: u32) -> &T {
...
//        &((*ptr).as_ref().unwrap().item)
//        if ptr.is_some() { return &((*ptr).as_ref().unwrap().item); }
//        else { return &0; }
    }
    */
}
