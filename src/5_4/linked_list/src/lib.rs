...
//impl<Idx, T> std::ops::Index<Idx> for LinkedList<T> where Idx: std::fmt::Debug + std::clone::Clone {
//impl<Idx, T> std::ops::Index<Idx> for LinkedList<T> where Idx: ?Sized + std::fmt::Debug {
//impl<Idx, T> std::ops::Index<Idx> for LinkedList<T> {
    type Output = T;
    fn index(&self, index: usize) -> &Self::Output {
//    fn index(&self, index: Idx) -> &Self::Output {
//    fn index(&self, index: Idx) where Idx: num::cast::AsPrimitive<i32> -> &Self::Output {
//    fn index<Idx: num::cast::AsPrimitive<i32>>(&self, index: Idx) -> &Self::Output {
//    fn index<Idx: num::cast::AsPrimitive<i32>>(&self, index: usize) -> &Self::Output {
//    fn index(&self, index: usize) -> &Self::Output { // error[E0053]: method `index` has an incompatible type for trait
//        fn get_ptr_from_idx<Idx, T>(ptr: &Option<Box<Node<T>>>, index: Idx, count: Idx) -> &Option<Box<Node<T>>> {
        fn get_ptr_from_idx<T>(ptr: &Option<Box<Node<T>>>, index: usize, count: usize) -> &Option<Box<Node<T>>> {
            if count < index {
                if ptr.is_some() { return get_ptr_from_idx(&ptr.as_ref().unwrap().next, index, count+1); }
                else { return ptr; }
...
//        if let Some(ref _n) = get_ptr_from_idx(&self.head, index, 0 as Idx) { return &(_n.item) }
        if let Some(ref _n) = get_ptr_from_idx(&self.head, index, 0) { return &(_n.item) }
        else { panic!("Out of index. index値が大きすぎます。index: {:?}", index); }
    }
}
/*
impl<usize, T> std::ops::Index<usize> for LinkedList<T> where usize: std::fmt::Debug + std::clone::Clone {
//impl<usize, T> std::ops::Index<usize> for LinkedList<T> where usize: ?Sized + std::fmt::Debug {
//impl<usize, T> std::ops::Index<usize> for LinkedList<T> {
    type Output = T;
    fn index(&self, index: usize) -> &Self::Output {
        fn get_ptr_from_idx<usize, T>(ptr: &Option<Box<Node<T>>>, index: usize, count: usize) -> &Option<Box<Node<T>>> {
            if count < index {
                if ptr.is_some() { return get_ptr_from_idx(&ptr.as_ref().unwrap().next, index, count+1); }
                else { return ptr; }
            } else { return ptr; }
        }
        if let Some(ref _n) = get_ptr_from_idx(&self.head, index, 0 as usize) { return &(_n.item) }
        else { panic!("Out of index. index値が大きすぎます。index: {:?}", index); }
    }
}
*/
